...
class Dao(object):
    ...
    __echo = True
    __coding = 'utf8'
    __rds_host = '182.254.245.224'
    __rds_port = 6379
    __rds_passwd = 'Yqc@2wsx'
    __session = None
    __redis = None
    __connect = None
    __media_con = None
    ...
    def init_db_uri(env=None):
        ...
        Dao.__db_uri = 'mysql://{}:{}@{}/{}?charset=utf8'.format(
            Dao.__config.mysql_user, Dao.__config.mysql_passwd,
        ...
        Dao.__rds_passwd = Dao.__config.redis_passwd

    @staticmethod
    def db_uri(user='root', host='127.0.0.1', password='123', db='yqc'):
    ...
    def media_engine(uri):
        """"""
        return create_engine(uri, echo=Dao.__config.mysql_echo, encoding=Dao.__coding)
        ...
        if Dao.__redis is None:
            Dao.__redis = redis.Redis(
                connection_pool=redis.ConnectionPool(
                    host=Dao.__config.redis_host,
                    port=Dao.__config.redis_port,
                    db=db,
                    password=Dao.__config.redis_passwd
                )
            )
        return Dao.__redis
